...
use antiyoy_agents::{SearchAgent, SearchConfig, SearchTurnSlate, reply_score, search_turn_slate};
    ...
    for map_index in 0..arguments.maps {
        ...
        while !game.is_terminal() && actions < arguments.action_limit {
            ...
            if game.round() % arguments.sample_round_modulus == arguments.sample_round_remainder {
                let record = record_slate(&game, map_generator.seed, scored);
...
fn score_slate(
    ...
    let scores = slate
        .turns
        .iter()
        .map(|turn| reply_score(turn, game.active_player(), reply_config))
        .collect::<Vec<_>>();
    let selected = slate
        .turns
        .iter()
        .enumerate()
        .max_by_key(|(index, turn)| (scores[*index], turn.score, Reverse(*index)))
        .map(|(index, _)| index)
...
fn record_slate(game: &Game, seed: u64, scored: ScoredSlate) -> SlateRecord {
    let games = scored
        .slate
        .turns
        .iter()
