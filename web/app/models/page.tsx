...
export default function ModelsPage() {
  return (
    <main className="models-page">
      ...
      <section className="models-section">
        <div className="section-heading"><div><p>Map fairness audit</p><h2>The seat is part of the experiment</h2></div><p>The same greedy bot played all five seats on 512 procedural maps per generator version. Schema 2 keeps each seed&apos;s geometry but rotates which seat inherits each starting position. These are self-play wins, not Elo or model-versus-model results.</p></div>
        <div className="ranking-table-wrap"><table className="ranking-table" style={{ minWidth: "34rem" }}><thead><tr><th>Seat</th><th>v1 wins / 512</th><th>v2 wins / 512</th><th>v1 mean start region</th><th>v2 mean start region</th></tr></thead><tbody>{benchmarkData.seatAudit.rows.map((row) => <tr key={row.seat} style={{ background: "transparent" }}><th scope="row">{row.seat + 1}</th><td>{row.legacyWins}</td><td>{row.rotatedWins}</td><td>{row.legacyRegion.toFixed(2)}</td><td>{row.rotatedRegion.toFixed(2)}</td></tr>)}</tbody></table></div>
        <p className="method-note">The initial region is a geometric Voronoi allocation, not owned territory. v1 ties the first generated position to seat 1; v2 distributes positions across seeds. A single map can still be unfair, and seat 1 retains a turn-order advantage. Original-position audit <SnapshotEvidenceLink evidence="procedural-seat-bias" /> · paired v1/v2 gate <SnapshotEvidenceLink evidence="rotated-seat-generator" />.</p>
        <p className="method-note">Model transfer scout: the frozen routed v6 five-player expert won 17 of 80 games on v2 maps, versus 16 for greedy on the same seeds and seats; 8 paired improvements, 7 regressions, p=1.0. No multiplayer strength promotion. <SnapshotEvidenceLink evidence="routed-rotated-scout" /></p>
      </section>
