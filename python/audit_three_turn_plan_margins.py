...
def summarize_overrides(rows: list[OverrideMargin]) -> dict[str, object]:
    ...
    return {
        ...
        "mean_selected_actions": float(
            np.mean([row.selected_length for row in rows])
        ),
...
def audit(fit_path: Path, source_path: Path, checkpoint_path: Path) -> dict[str, object]:
...
def main() -> None:
    ...
    print(json.dumps(audit(arguments.fit, arguments.source, arguments.checkpoint), sort_keys=True))
